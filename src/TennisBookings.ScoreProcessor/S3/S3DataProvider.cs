...
public class S3DataProvider : IS3DataProvider
{
		...
		string objectKey, CancellationToken cancellationToken = default)
	{
		try
		{
			...
			using var response = await _amazonS3
		...
		{
			_logger.LogError(e, "An exception occurred while reading the " +
				"S3 object into the memory stream.");
		}
