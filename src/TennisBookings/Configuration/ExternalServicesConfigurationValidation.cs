	...
	IValidateOptions<ExternalServicesConfiguration>
{
		...
		ExternalServicesConfiguration options)
    {
        ...
        {
            case "WeatherApi":
                var result = ValidateWeatherApiConfig(
					_weatherConfig.EnableWeatherForecast, options);
                if (result.Failed)
                    return result;
                break;
            ...
            case "ProductsApi":
                ...
                {
                    return ValidateOptionsResult.Fail("A URL for the product " +
						"API is required.");
                }
                if (options.MinsToCache < 1)
                {
                    return ValidateOptionsResult.Fail("The products caching must " +
		...
		bool weatherForecastEnabled, ExternalServicesConfiguration options)
    {
        ...
        {
            return ValidateOptionsResult.Fail("A URL for the weather API is " +
				"required when weather forecasting is enabled.");
        }
        if (options.MinsToCache < 10)
        {
            return ValidateOptionsResult.Fail("The weather forecast cachine " +
